...
<script type="text/javascript">
    ...
    function livePayments(){
        $.ajax({
            url: "/api/mobile/wallet/list?q=<?=auth()->id()?>",
            type: 'GET',
            processData: true,
    ...
    function checkIfNew(pays){
        // for(let i=1;i<pays.length;i++){
        //     let pay = pays[i];
        //     if(!payments.includes(pay['transaction_reference'])){
        //         payments.push(pay['transaction_reference']);
        //         alert(
        //           "Payment received\n\n" +
        //             "Customer: "+pay['first_name']+" "+pay['last_name']+"\n" +
        //             "Amount: "+pay['amount']+"\n"+
        //             "Reference: "+pay['transaction_reference']+"\n" +
        //             "Phone Number: " +pay["sender_phone"]+"\n"+
        //             "Time Received: "+pay['transaction_timestamp']
        //         );
        //         break;
        //     }
        // }
        ...
        var lastElement = pays.length - 1;
        let pay = pays[lastElement];
        //  fix. not tested
        console.log('pay is')
        
        var timeToCompare = new Date(Date.now() - 10000).getTime()
        console.log(timeToCompare)
        var created_at = new Date(pay['transaction_timestamp']).getTime()
        
        if(created_at >= timeToCompare){
